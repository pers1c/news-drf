...
class CategoryDetailView(generics.RetrieveUpdateDestroyAPIView):
    '''API endpoint для конкретной категрии'''
    queryset = Category.objects.all()
    serializer_class = CategorySerializer
    permission_classes = [permissions.IsAuthenticatedOrReadOnly]
    lookup_field = 'slug'
...
class PostListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        # фильтрация поп правам доступа
        if not self.request.user.is_authenticated:
            queryset = queryset.filter(status='published')
        else:
            queryset = queryset.filter(status=Q(status='published') | Q(author=self.request.user))
        return queryset
...
class PostDetailView(generics.RetrieveUpdateDestroyAPIView):
    '''API endpoitn для конкретного поста'''
    queryset = Post.objects.select_related('author', 'category')
...
class MyPostsView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Post.objects.filter(
            author=self.request.user,).select_related('author', 'category')
...
def recent_posts(request):
    '''10 послених опубликованных постов'''
    posts = Post.objects.filter(
        status='published'
